document.addEventListener('DOMContentLoaded', function() {
  ...
  Tutorial.show({
    ...
    subtitle: 'ÉPREUVE 1',
    description: 'Inspecte les situations sur le chantier et signale les dangers éventuels.',
    steps: [
      { icon: '👀', text: 'Lis attentivement la situation qui s\'affiche.' },
      { icon: '⚠️', text: 'Clique sur <strong>DANGER</strong> si la sécurité n\'est pas respectée.' },
      { icon: '✅', text: 'Clique sur <strong>OK</strong> si tout est conforme.' },
      { icon: '🎯', text: 'Obtiens 8 bonnes réponses sur 10 pour valider.' }
    ],
  ...
  function initGame() {
    ...
    // Cloning buttons to avoid double execution on reload
    if (btnDanger) {
      var newBtnDanger = btnDanger.cloneNode(true);
      btnDanger.parentNode.replaceChild(newBtnDanger, btnDanger);
      newBtnDanger.addEventListener('click', function() { handleChoice(true); });
    }
    if (btnOk) {
      var newBtnOk = btnOk.cloneNode(true);
      btnOk.parentNode.replaceChild(newBtnOk, btnOk);
      newBtnOk.addEventListener('click', function() { handleChoice(false); });
    }
  ...
  function showResult(success) {
    ...
    // Pour obtenir le score même si la page est rechargée (on cache s'il n'est pas dispo)
    if (resultScore) {
      var total = 10;
      var scoreTxt = getGameState().enigme1 && getGameState().enigme1.completed ? 'Validé' : 'Échoué';
      resultScore.textContent = 'Résultat : ' + scoreTxt; 
    }
